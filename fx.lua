...
-- particle colours per element, and shape: {w,h,radius}
...
-- put a box at (x,y) inside the 50x50 sprite area of the given side
...
local function particles(en,h)
  local pc,ps=PC[pat],PS[pat]
  for i=1,6 do
    local b=pbox(i)
    local w,hh=ps[1],ps[2]
    local x,y
    if pat=="fire" or pat=="water" then x=4+((i*13+h//60)%40) y=46-((h//7+i*9)%46)
    elseif pat=="grass" then x=2+((i*11+h//40)%40) y=((h//8+i*9)%46)
    ...
    put(b,en,x,y,w,hh)
    b:hidden(pat=="elec" and badge.sys.random(3)==0)
  end
end
